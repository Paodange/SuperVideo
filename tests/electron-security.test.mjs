...
test("Agent IPC uses explicit channels, payload schemas, and stable public errors", async () => {
  ...
  assert.equal(fakeIpcMain.handlers.size, 27);
...
test("preload exposes one frozen typed capability and no arbitrary channel", async () => {
  ...
  assert.deepEqual(Object.keys(api), [
    ...
    "createProject", "openProject", "addAssetReferences", "listProjectAssets", "inspectSentenceQa", "saveSentenceQa", "retrieveSentences", "rerankSentences", "alignScript", "createRemixPlan", "optimizeDuration", "cutJoinAroll", "planSubtitles", "renderPreview", "checkPreviewQuality", "exportFinalMp4", "editTimeline",
