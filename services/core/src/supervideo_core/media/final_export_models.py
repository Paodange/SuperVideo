...
FINAL_EXPORT_POLICY = "verified-preview-mux-v1"
...
AUDIO_OUTPUT_PATTERN = re.compile(r"^previews/aroll-cut-join-v1/[0-9a-f]{64}\.audio\.m4a$")
...
class FinalMp4ExportParams(FinalExportModel):
    ...
    audio_result: ArollCutJoinResult = Field(alias="audioResult")
    audio_fingerprint: str = Field(alias="audioFingerprint", min_length=64, max_length=64)
    ...
    @classmethod
    def validate_audio_fingerprint(cls, value: str) -> str:
        if SHA256_PATTERN.fullmatch(value) is None:
            raise ValueError("invalid final export audio fingerprint")
        return value
    ...
    def validate_input(self) -> "FinalMp4ExportParams":
        ...
        if self.audio_result.project_id != self.project_id or self.audio_result.timeline_id != self.preview_result.timeline_id:
            raise ValueError("final export audio result belongs to another project or timeline")
...
class FinalMp4Container(FinalExportModel):
    ...
    @model_validator(mode="after")
    def validate_formal_container(self) -> "FinalMp4Container":
        if "mp4" not in {part.strip().casefold() for part in self.format_name.split(",")} or self.video_codec != "h264" or self.audio_codec != "aac" or self.width != 1080 or self.height != 1920:
            raise ValueError("final output must be 1080x1920 H.264/AAC")
        return self
...
class FinalMp4Manifest(FinalExportModel):
    ...
    export_policy: Literal["verified-preview-mux-v1"] = Field(alias="exportPolicy")
    ...
    audio_plan_digest: str = Field(alias="audioPlanDigest", min_length=64, max_length=64)
    audio_relative_path: str = Field(alias="audioRelativePath", min_length=1, max_length=512)
    audio_output_fingerprint: str = Field(alias="audioOutputFingerprint", min_length=64, max_length=64)
    ...
    @field_validator("plan_digest", "preview_output_fingerprint", "quality_digest", "audio_plan_digest", "audio_output_fingerprint", "output_fingerprint")
    @classmethod
    ...
    def validate_manifest_paths(self) -> "FinalMp4Manifest":
        ...
        if self.manifest_relative_path != self.relative_path.removesuffix(".mp4") + ".manifest.json":
            raise ValueError("final manifest path does not match output path")
        if AUDIO_OUTPUT_PATTERN.fullmatch(self.audio_relative_path) is None:
            raise ValueError("final manifest audio path is outside the preview boundary")
        return self
...
class FinalMp4ExportResult(FinalExportModel):
    ...
    @field_validator("plan_digest", "quality_digest", "audio_plan_digest", "audio_fingerprint")
    @classmethod
    def validate_digest(cls, value: str) -> str:
        if SHA256_PATTERN.fullmatch(value) is None:
