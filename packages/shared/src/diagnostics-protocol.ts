...
export const DIAGNOSTICS_MAX_WORKER_CAPABILITIES = 32;
...
export function isDiagnosticDocument(value: unknown): value is DiagnosticDocument {
  ...
  if (!isPlainRecord(worker) || !hasExactKeys(worker, ["status", "protocolVersion", "generation", "workerVersion", "capabilityCount", "restartCount", "lastErrorCode"]) || !isSafeString(worker.status, 32) || !isSafeInteger(worker.protocolVersion, 1, 1) || !isSafeInteger(worker.generation, 0, Number.MAX_SAFE_INTEGER) || !(worker.workerVersion === null || isSafeString(worker.workerVersion, 32)) || !isSafeInteger(worker.capabilityCount, 0, DIAGNOSTICS_MAX_WORKER_CAPABILITIES) || !isSafeInteger(worker.restartCount, 0, Number.MAX_SAFE_INTEGER) || !(worker.lastErrorCode === null || isSafeString(worker.lastErrorCode, 128))) return false;
  if (!isPlainRecord(core) || !hasExactKeys(core, ["status", "protocolVersion", "coreVersion", "capabilityCount"]) || !isSafeString(core.status, 32) || !isSafeInteger(core.protocolVersion, 1, 1) || !(core.coreVersion === null || isSafeString(core.coreVersion, 32)) || !isSafeInteger(core.capabilityCount, 0, 64)) return false;
